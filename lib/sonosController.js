...
class SonosController extends EventEmitter {
  ...
  async start() {
    ...
    try {
      ...
      this.manager.InitializeWithDiscovery(15)
.then(this.app.debug)                                                                                                      .then(() => {                                                                                                             manager.Devices.forEach(d => this.app.debug('Device %s (%s) is joined in %s', d.Name, d.uuid, d.GroupName))
